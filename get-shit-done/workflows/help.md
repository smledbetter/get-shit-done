...
**`/gsd:sprint [range] [--dry-run] [--skip-failures] [--consolidated] [--prd <file>]`**
Run multiple phases end-to-end without manual intervention.

- Chains discuss → plan → execute → verify for each phase automatically
- Handles failures: retries gap closure, then stops or skips (configurable)
- Produces `SPRINT-REPORT.md` with per-phase results
- Respects consolidated mode and PRD express path
- Does not cross milestone boundaries

Usage:
- `/gsd:sprint` — run all remaining phases
- `/gsd:sprint 3-5` — run phases 3 through 5
- `/gsd:sprint --dry-run` — preview what would run
- `/gsd:sprint --skip-failures` — log failures, continue to next phase
- `/gsd:sprint --consolidated --prd spec.md` — fastest path

Configure default failure behavior via `/gsd:settings` → Sprint.
...
├── SPRINT-REPORT.md      # Last sprint results (generated by /gsd:sprint)
