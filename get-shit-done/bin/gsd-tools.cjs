...
function metricsGetSessionLogsDir(cwd) {
  const homedir = require("os").homedir();
  // Claude Code stores session logs keyed by CWD path with / replaced by -
  const slug = cwd.replace(/\//g, "-");
  return path.join(homedir, ".claude", "projects", slug);
}

function metricsParseSessionLogs(logsDir) {
...
function metricsGetPhaseBoundaries(cwd) {
  // Use git commit timestamps to find phase boundaries
  // Phase commits match patterns like "phase-NN" or "(phase-NN)" in commit messages
  try {
    const log = execSync('git log --all --format="%H|%aI|%s" --reverse', {
      cwd,
      encoding: "utf-8",
    });
    const boundaries = [];
    for (const line of log.split("\n")) {
      ...
      // Match phase references: "phase-1", "(phase-2)", "phase 3"
      const phaseMatch = msg.match(/phase[- ](\d+(?:\.\d+)?)/i);
      if (phaseMatch) {
        boundaries.push({
          phase: phaseMatch[1],
          timestamp,
...
function metricsAssignCallsToPhases(calls, boundaries) {
  // Build phase time ranges from boundaries
  const phases = {};
  const phaseNums = [...new Set(boundaries.map((b) => b.phase))];

  for (const phaseNum of phaseNums) {
    const phaseBoundaries = boundaries.filter((b) => b.phase === phaseNum);
    const firstCommit = phaseBoundaries[0];
    const lastCommit = phaseBoundaries[phaseBoundaries.length - 1];
    phases[phaseNum] = {
  ...
  // Assign calls to phases based on timestamp ranges
  // A call belongs to a phase if it falls between that phase's first commit
  // and the next phase's first commit (or end of time)
  const result = {};
  for (let i = 0; i < sortedPhases.length; i++) {
    const [phaseNum, range] = sortedPhases[i];
    const nextStart =
      i + 1 < sortedPhases.length ? sortedPhases[i + 1][1].start : null;

    // Extend start back to capture calls before the first commit of this phase
    // (work happens before the commit). Use previous phase's last commit or beginning of time.
    const prevEnd = i > 0 ? sortedPhases[i - 1][1].end : null;
    const effectiveStart = prevEnd || "1970-01-01T00:00:00Z";
    const effectiveEnd = nextStart || "9999-12-31T23:59:59Z";

    result[phaseNum] = calls.filter(
      (c) => c.timestamp >= effectiveStart && c.timestamp < effectiveEnd,
    );
  }

  // Calls not assigned to any phase go to "unassigned"
  const assignedTimestamps = new Set(
    Object.values(result)
      .flat()
      .map((c) => c.timestamp),
  );
  const unassigned = calls.filter((c) => !assignedTimestamps.has(c.timestamp));
...
function metricsAggregate(calls) {
  const totals = {
  ...
  for (const call of calls) {
    ...
    totals.cache_creation_input_tokens += call.cache_creation_input_tokens;
    totals.cache_read_input_tokens += call.cache_read_input_tokens;

    const model = call.model.replace(/^claude-/, "").replace(/-\d{8}$/, "");
  ...
  totals.new_work_tokens =
    totals.input_tokens +
    totals.output_tokens +
    totals.cache_creation_input_tokens;
  totals.cache_ratio =
    totals.total_tokens > 0
      ? ((totals.cache_read_input_tokens / totals.total_tokens) * 100).toFixed(
          1,
        ) + "%"
      : "0%";
  totals.new_work_pct =
    totals.total_tokens > 0
      ? ((totals.new_work_tokens / totals.total_tokens) * 100).toFixed(1) + "%"
      : "0%";

  // Model mix percentages
  totals.model_mix = {};
  for (const [model, count] of Object.entries(totals.models)) {
    totals.model_mix[model] = {
      calls: count,
      pct: ((count / totals.api_calls) * 100).toFixed(1) + "%",
    };
  }
...
function cmdMetricsPhase(cwd, phaseNum, raw) {
  if (!phaseNum) {
    error("Phase number required. Usage: metrics phase <number>");
  }

  const logsDir = metricsGetSessionLogsDir(cwd);
  const calls = metricsParseSessionLogs(logsDir);
  if (calls.length === 0) {
    output({ error: "No session logs found", logs_dir: logsDir }, raw);
    return;
  }

  const boundaries = metricsGetPhaseBoundaries(cwd);
  const byPhase = metricsAssignCallsToPhases(calls, boundaries);
  const phaseCalls = byPhase[phaseNum];

  if (!phaseCalls || phaseCalls.length === 0) {
    output(
      {
        error: `No data found for phase ${phaseNum}`,
        available_phases: Object.keys(byPhase).filter(
          (k) => k !== "unassigned",
        ),
      },
      raw,
  ...
  const totals = metricsAggregate(phaseCalls);
  output(
    {
      phase: phaseNum,
      ...totals,
      data_source: logsDir,
      session_count: new Set(phaseCalls.map((c) => c.sessionId)).size,
    },
    raw,
  );
}

function cmdMetricsMilestone(cwd, raw) {
  const logsDir = metricsGetSessionLogsDir(cwd);
  const calls = metricsParseSessionLogs(logsDir);
  if (calls.length === 0) {
    output({ error: "No session logs found", logs_dir: logsDir }, raw);
  ...
  const phases = {};
  for (const [phaseNum, phaseCalls] of Object.entries(byPhase)) {
    if (phaseNum === "unassigned") continue;
    phases[phaseNum] = metricsAggregate(phaseCalls);
  }

  const totals = metricsAggregate(calls);
  output(
    {
      scope: "milestone",
      phases,
      totals,
      data_source: logsDir,
      session_count: new Set(calls.map((c) => c.sessionId)).size,
    },
    raw,
  );
}
  ...
  output(
    {
      ...
      phases,
      totals,
      time_span: { first: firstCall.timestamp, last: lastCall.timestamp },
      data_source: logsDir,
      session_count: new Set(calls.map((c) => c.sessionId)).size,
      log_files: fs.readdirSync(logsDir).filter((f) => f.endsWith(".jsonl"))
        .length,
    },
    raw,
  );
...
function cmdMetricsSnapshot(cwd, phaseNum, raw) {
  if (!phaseNum) {
    error("Phase number required. Usage: metrics snapshot <number>");
  }
  ...
  const snapshotPath = path.join(metricsDir, `phase-${phaseNum}.json`);
...
async function main() {
  ...
  if (!command) {
    error(
      "Usage: gsd-tools <command> [args] [--raw]\nCommands: state, resolve-model, find-phase, commit, verify-summary, verify, frontmatter, template, generate-slug, current-timestamp, list-todos, verify-path-exists, config-ensure-section, metrics, init",
    );
  }
  ...
  switch (command) {
    ...
    case "metrics": {
      const subcommand = args[1];
      if (subcommand === "phase") {
        cmdMetricsPhase(cwd, args[2], raw);
      } else if (subcommand === "milestone") {
        cmdMetricsMilestone(cwd, raw);
      } else if (subcommand === "project") {
        cmdMetricsProject(cwd, raw);
      } else if (subcommand === "snapshot") {
        cmdMetricsSnapshot(cwd, args[2], raw);
      } else {
        error(
          "Unknown metrics subcommand. Available: phase, milestone, project, snapshot",
        );
      }
      break;
    }
