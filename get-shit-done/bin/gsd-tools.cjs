 ...
 *     --phase N [--fields '{json}']
 *
 * Metrics (Token Efficiency Tracking):
 *   metrics phase <N> [--plan M]       Token breakdown for phase N
 *   metrics milestone [version]        Aggregate for milestone
 *   metrics project                    Full project lifetime totals
 *   metrics snapshot <phase>           Save phase snapshot to .planning/metrics/
 *   metrics milestone-snapshot [ver]   Save milestone snapshot
 *
 * State Progression:
...
function loadConfig(cwd) {
  ...
  try {
    ...
    return {
      ...
      commit_docs:
        get("commit_docs", { section: "planning", field: "commit_docs" }) ??
...
function isGitIgnored(cwd, targetPath) {
  try {
    execSync(
      "git check-ignore -q -- " + targetPath.replace(/[^a-zA-Z0-9._\-/]/g, ""),
      {
        cwd,
        stdio: "pipe",
      },
...
function cmdStateAdvancePlan(cwd, raw) {
  ...
  if (currentPlan >= totalPlans) {
    ...
    content =
      stateReplaceField(content, "Status", "Ready to execute") || content;
    content = stateReplaceField(content, "Last Activity", today) || content;
    fs.writeFileSync(statePath, content, "utf-8");
    output(
      {
...
function cmdStateRecordMetric(cwd, options, raw) {
  ...
  let content = fs.readFileSync(statePath, "utf-8");
  const { phase, plan, duration, tasks, files, tokens, calls } = options;
  ...
  if (metricsMatch) {
    ...
    // Format token count for display (e.g., 1.5M, 234K)
    const fmtTokens = (n) => {
      if (!n || n === "-") return "-";
    ...
    const tokensCol = tokens ? ` ${fmtTokens(tokens)} tokens |` : "";
    const callsCol = calls ? ` ${calls} calls |` : "";
    const newRow = `| Phase ${phase} P${plan} | ${duration} | ${tasks || "-"} tasks | ${files || "-"} files |${tokensCol}${callsCol}`;
...
function cmdStateUpdateProgress(cwd, raw) {
  ...
  const percent =
    totalPlans > 0 ? Math.round((totalSummaries / totalPlans) * 100) : 0;
  const barWidth = 10;
...
function cmdTemplateFill(cwd, templateType, options, raw) {
  ...
  if (fs.existsSync(outPath)) {
    output(
      { error: "File already exists", path: path.relative(cwd, outPath) },
      raw,
    );
    return;
  }
...
function cmdStateSnapshot(cwd, raw) {
  ...
  if (sessionMatch) {
    ...
    const resumeFileMatch = sessionSection.match(
      /\*\*Resume File:\*\*\s*(.+)/i,
    );
  ...
  if (fs.existsSync(roadmapPath)) {
    ...
    if (fs.existsSync(reqPath)) {
      ...
      const reqMatch = roadmapContent.match(
        new RegExp(
          `Phase\\s+${phaseNum.replace(".", "\\.")}[\\s\\S]*?\\*\\*Requirements:\\*\\*\\s*([^\\n]+)`,
          "i",
        ),
      );
...
function cmdInitPlanPhase(cwd, phase, includes, raw) {
  ...
  if (includes.has("requirements")) {
    result.requirements_content = safeReadFile(
  ...
  if (includes.has("context") && phaseInfo?.directory) {
    ...
    try {
      ...
      if (contextFile) {
        result.context_content = safeReadFile(
          path.join(phaseDirFull, contextFile),
        );
      }
    } catch {}
  }
  if (includes.has("research") && phaseInfo?.directory) {
    ...
    try {
      ...
      if (researchFile) {
        result.research_content = safeReadFile(
          path.join(phaseDirFull, researchFile),
        );
      }
    } catch {}
  }
  if (includes.has("verification") && phaseInfo?.directory) {
    ...
    try {
      ...
      if (verificationFile) {
        result.verification_content = safeReadFile(
          path.join(phaseDirFull, verificationFile),
        );
      }
    } catch {}
  }
...
function cmdInitResume(cwd, raw) {
  ...
  try {
    interruptedAgentId = fs
      .readFileSync(
        path.join(cwd, ".planning", "current-agent-id.txt"),
        "utf-8",
      )
      .trim();
  } catch {}
...
function cmdInitProgress(cwd, includes, raw) {
  ...
  if (includes.has("project")) {
    result.project_content = safeReadFile(
...
/**
 * Derive the Claude Code project slug from a directory path.
 * Claude Code stores logs at ~/.claude/projects/{slug}/ where slug is
 * the absolute path with / replaced by - and prefixed with -.
 */
function metricsProjectSlug(cwd) {
  // Claude Code replaces / and non-alphanumeric chars (except -) with -
  return (
    "-" +
    cwd
      .replace(/\//g, "-")
...
/**
 * Parse all JSONL session logs for a project.
 * Returns array of {timestamp, model, input, output, cache_read, cache_create}.
 */
function metricsParseSessionLogs(slugDir) {
  const entries = [];
  if (!fs.existsSync(slugDir)) return entries;

  // Collect all .jsonl files: top-level + subagent directories
  const jsonlFiles = [];
  const topFiles = fs.readdirSync(slugDir);
      ...
      if (fs.existsSync(subDir)) {
        try {
          ...
          for (const sf of subs) {
            if (sf.endsWith(".jsonl")) jsonlFiles.push(path.join(subDir, sf));
          }
        } catch {}
      }
    }
...
/**
 * Normalize model name to short form: "opus", "sonnet", "haiku", or the raw name.
 */
function metricsNormalizeModel(model) {
...
/**
 * Detect phase time boundaries from git commit messages.
 * Looks for commits matching ({phaseNum}- pattern to find first/last timestamps.
 * Returns {start, end} ISO timestamps or null if no commits found.
 */
function metricsDetectPhaseBoundary(cwd, phaseNum) {
  try {
    // Match commits like feat(01-01):, fix(1-02):, test(01-01): etc.
    const padded = String(phaseNum).padStart(2, "0");
    const log = execSync(
      `git log --all --grep="(${padded}-" --format="%aI" --date=iso-strict 2>/dev/null || true`,
...
/**
 * Detect all phase boundaries for the project.
 * Returns Map<phaseNumber, {start, end, name}>.
 */
function metricsDetectAllBoundaries(cwd) {
  const boundaries = new Map();
  const phasesDir = path.join(cwd, ".planning", "phases");
  if (!fs.existsSync(phasesDir)) return boundaries;
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      const phaseNum = match[1];
      const phaseName = dir;
      const boundary = metricsDetectPhaseBoundary(cwd, phaseNum);
      if (boundary) {
        boundaries.set(phaseNum, { ...boundary, name: phaseName });
      }
    }
  } catch {}
...
/**
 * Classify log entries into phases by timestamp.
 * Entries outside any phase boundary go into "unclassified".
 */
function metricsClassifyByPhase(entries, boundaries) {
  const classified = new Map();

  // Sort boundaries by start time for efficient classification
  const sortedPhases = [...boundaries.entries()].sort((a, b) =>
    a[1].start.localeCompare(b[1].start),
  );

  for (const entry of entries) {
    let assigned = "unclassified";
    for (const [phaseNum, bound] of sortedPhases) {
      if (entry.timestamp >= bound.start && entry.timestamp <= bound.end) {
        assigned = phaseNum;
        break;
      }
    }
    if (!classified.has(assigned)) classified.set(assigned, []);
    classified.get(assigned).push(entry);
  }
...
/**
 * Aggregate token data from a set of log entries.
 * Returns summary object with totals, efficiency metrics, and model breakdown.
 */
function metricsAggregate(entries) {
  const totals = {
    input: 0,
    output: 0,
    cache_read: 0,
  ...
  for (const e of entries) {
    ...
    const short = metricsNormalizeModel(e.model);
    if (!models[short]) models[short] = { calls: 0, tokens: 0 };
    models[short].calls++;
    models[short].tokens += e.input + e.output + e.cache_read + e.cache_create;
  }

  totals.total =
    totals.input + totals.output + totals.cache_read + totals.cache_create;
  totals.new_work = totals.input + totals.output + totals.cache_create;

  const cache_ratio =
    totals.new_work > 0 ? +(totals.cache_read / totals.new_work).toFixed(1) : 0;
  const new_work_pct =
    totals.total > 0 ? +((totals.new_work / totals.total) * 100).toFixed(1) : 0;

  // Model mix percentages
  const model_mix = {};
  for (const [m, data] of Object.entries(models)) {
    model_mix[m] = {
      calls: data.calls,
      tokens: data.tokens,
      pct: calls > 0 ? +((data.calls / calls) * 100).toFixed(1) : 0,
    };
  }
  ...
  return {
    api_calls: calls,
    tokens: totals,
    efficiency: { cache_ratio, new_work_pct },
    models: model_mix,
  };
}
...
/**
 * Format token count in human-readable form (e.g., 1.5M, 234K).
 */
...
function cmdMetricsPhase(cwd, phaseNum, options, raw) {
  const slug = metricsProjectSlug(cwd);
  ...
  const entries = metricsParseSessionLogs(slugDir);

  if (entries.length === 0) {
    output({ error: "No session logs found", slug_dir: slugDir }, raw);
    return;
  }
  ...
  if (!boundary) {
    output(
      { error: `No commits found for phase ${phaseNum}`, phase: phaseNum },
      raw,
    );
    return;
  }

  // Filter entries within this phase's time range
  const phaseEntries = entries.filter(
    (e) => e.timestamp >= boundary.start && e.timestamp <= boundary.end,
  );

  // If --plan is specified, try to narrow further using plan commit boundaries
  if (options && options.plan) {
    const planNum = String(options.plan).padStart(2, "0");
    try {
      const planLog = execSync(
        `git log --all --grep="(${padded}-${planNum})" --format="%aI" --date=iso-strict 2>/dev/null || true`,
        { cwd, encoding: "utf-8", timeout: 10000 },
      ).trim();
      if (planLog) {
        const planTs = planLog.split("\n").filter(Boolean).sort();
        if (planTs.length > 0) {
          const planStart = planTs[0];
          const planEnd = planTs[planTs.length - 1];
          const planEntries = phaseEntries.filter(
            (e) => e.timestamp >= planStart && e.timestamp <= planEnd,
          );
          const agg = metricsAggregate(planEntries);
          agg.phase = phaseNum;
          agg.plan = options.plan;
          agg.time_range = { start: planStart, end: planEnd };
          output(agg, raw);
          return;
        }
      }
  ...
  const agg = metricsAggregate(phaseEntries);
  agg.phase = phaseNum;
  agg.time_range = boundary;
  output(agg, raw);
}

function cmdMetricsMilestone(cwd, version, raw) {
  const slug = metricsProjectSlug(cwd);
  const slugDir = path.join(
    require("os").homedir(),
    ".claude",
  ...
  const boundaries = metricsDetectAllBoundaries(cwd);
  if (boundaries.size === 0) {
    output({ error: "No phase boundaries detected from commits" }, raw);
    return;
  }

  const classified = metricsClassifyByPhase(entries, boundaries);

  // Aggregate per phase and total
  const phases = {};
  const allPhaseEntries = [];
  for (const [phaseNum, bound] of boundaries) {
    const phaseEntries = classified.get(phaseNum) || [];
    phases[phaseNum] = {
      name: bound.name,
      ...metricsAggregate(phaseEntries),
      time_range: { start: bound.start, end: bound.end },
    };
    allPhaseEntries.push(...phaseEntries);
  }

  const totals = metricsAggregate(allPhaseEntries);
  const unclassified = classified.get("unclassified") || [];

  output(
    {
      milestone: version || "current",
      phase_count: boundaries.size,
      phases,
      totals,
      unclassified_calls: unclassified.length,
    },
    raw,
  );
}

function cmdMetricsProject(cwd, raw) {
  const slug = metricsProjectSlug(cwd);
  const slugDir = path.join(
    require("os").homedir(),
    ".claude",
  ...
  // Grand totals from ALL entries (including unclassified)
  const totals = metricsAggregate(entries);
  const unclassified = classified.get("unclassified") || [];
...
function cmdMetricsSnapshot(cwd, phaseNum, raw) {
  const slug = metricsProjectSlug(cwd);
  const slugDir = path.join(
    require("os").homedir(),
    ".claude",
  ...
  if (entries.length === 0) {
    output({ error: "No session logs found" }, raw);
    return;
  }

  const boundary = metricsDetectPhaseBoundary(cwd, phaseNum);
  if (!boundary) {
    output({ error: `No commits found for phase ${phaseNum}` }, raw);
    return;
  }
  ...
  // Get git stats for this phase
  const padded = String(phaseNum).padStart(2, "0");
  let gitStats = { commits: 0, files_changed: 0 };
  try {
    const commitCount = execSync(
      `git log --all --grep="(${padded}-" --oneline 2>/dev/null | wc -l`,
      { cwd, encoding: "utf-8", timeout: 10000 },
    ).trim();
    gitStats.commits = parseInt(commitCount, 10) || 0;

    const filesChanged = execSync(
      `git log --all --grep="(${padded}-" --diff-filter=ACMR --name-only --format="" 2>/dev/null | sort -u | wc -l`,
      { cwd, encoding: "utf-8", timeout: 10000 },
    ).trim();
    gitStats.files_changed = parseInt(filesChanged, 10) || 0;
  } catch {}

  // Find phase name from directory
  let phaseName = padded;
  const phasesDir = path.join(cwd, ".planning", "phases");
  try {
    const dirs = fs.readdirSync(phasesDir).filter((d) => d.startsWith(padded));
    if (dirs.length > 0) phaseName = dirs[0];
  } catch {}
  ...
  const snapshot = {
    ...
    efficiency: {
      ...
      tokens_per_commit:
        gitStats.commits > 0
          ? Math.round(agg.tokens.new_work / gitStats.commits)
          : 0,
    },
    git: gitStats,
  };

  // Write snapshot to .planning/metrics/
  const metricsDir = path.join(cwd, ".planning", "metrics");
  if (!fs.existsSync(metricsDir)) fs.mkdirSync(metricsDir, { recursive: true });

  const snapshotPath = path.join(metricsDir, `phase-${padded}.json`);
  fs.writeFileSync(snapshotPath, JSON.stringify(snapshot, null, 2), "utf-8");

  output({ snapshot_path: snapshotPath, ...snapshot }, raw);
}

function cmdMetricsMilestoneSnapshot(cwd, version, raw) {
  const slug = metricsProjectSlug(cwd);
  const slugDir = path.join(
    require("os").homedir(),
    ".claude",
  ...
  for (const [phaseNum, bound] of boundaries) {
    const phaseEntries = classified.get(phaseNum) || [];
    phases[phaseNum] = metricsAggregate(phaseEntries);
    allPhaseEntries.push(...phaseEntries);
  }
  ...
  const safeName = (version || "current").replace(/[^a-zA-Z0-9.-]/g, "_");
  const snapshotPath = path.join(metricsDir, `milestone-${safeName}.json`);
  fs.writeFileSync(snapshotPath, JSON.stringify(snapshot, null, 2), "utf-8");
  ...
  switch (command) {
      ...
      if (subcommand === "update") {
        ...
        cmdStateRecordMetric(
          cwd,
          {
            ...
            tokens: tokensIdx !== -1 ? args[tokensIdx + 1] : null,
            calls: callsIdx !== -1 ? args[callsIdx + 1] : null,
          },
          raw,
        );
    ...
    case "milestone": {
      ...
      if (subcommand === "complete") {
        ...
        cmdMilestoneComplete(
          cwd,
          args[2],
          { name: milestoneName, archivePhases },
          raw,
        );
      } else {
    ...
    case "metrics": {
      ...
      if (subcommand === "phase") {
        ...
        const planIdx = args.indexOf("--plan");
        const planOpt = planIdx !== -1 ? args[planIdx + 1] : null;
        if (!phaseNum) error("Usage: metrics phase <N> [--plan M]");
        cmdMetricsPhase(cwd, phaseNum, { plan: planOpt }, raw);
      } else if (subcommand === "milestone") {
        cmdMetricsMilestone(cwd, args[2] || null, raw);
      } else if (subcommand === "project") {
        cmdMetricsProject(cwd, raw);
      } else if (subcommand === "snapshot") {
        const phaseNum = args[2];
        if (!phaseNum) error("Usage: metrics snapshot <phase>");
        cmdMetricsSnapshot(cwd, phaseNum, raw);
      } else if (subcommand === "milestone-snapshot") {
        cmdMetricsMilestoneSnapshot(cwd, args[2] || null, raw);
      } else {
        error(
          "Unknown metrics subcommand. Available: phase, milestone, project, snapshot, milestone-snapshot",
        );
      }
      break;
    }
